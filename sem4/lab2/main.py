...
class TruthTable:
    def __init__(self, expression):
        ...
        # self.expression = expression.replace('&', ' and ').replace('!', ' not ').replace('not',' not ').replace('and',' and ').replace('|',' or ').replace('->',' <= ').replace('~','==')
        self.expression = expression
    def evaluate(self, values):
        ...
        for i in opz:
            if i in ['a', 'b', 'c', 'd', 'e']:
                ...
                # rstring = str(self.rstack.pop()) +str(i) + str(self.rstack.pop())
